...
class FertilizerDetailsActivity : AppCompatActivity() {
    lateinit var binding: ActivityFertilizerDetailsBinding
    lateinit var fertilizerId : String

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityFertilizerDetailsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        fertilizerId = intent.getStringExtra("fertilizerId").toString()
    ...
    private fun showBottomSheet() {
        ...
        MyClass().fetchSingleData("Fertilizer",fertilizerId, Fertilizer::class.java, onDataChange = { seed->
            if(seed!=null){
